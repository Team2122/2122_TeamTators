...
public abstract class Command implements CommandRunContext {
    ...
    protected void initialize() {
        // Why is this needed, we have the local debug version for logging ???
        logger.debug("{} initializing", getName());
    }

    protected void initialize(boolean localDebug) {
        if (localDebug) {
            logger.debug("{} initializing", getName());
        }
    }
    ...
    /**
     * do a tick in the command (approximately 50 of these happen per second)
    ...
    protected void finish(boolean interrupted, boolean localDebug) {
        if (localDebug) {
            if (interrupted) {
                logger.debug("{} interrupted", getName());
            } else {
