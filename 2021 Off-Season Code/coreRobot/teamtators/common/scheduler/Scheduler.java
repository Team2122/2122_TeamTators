...
public final class Scheduler implements CommandRunContext, RobotStateListener, FMSDataListener {
    ...
    public Map<String, CommandRun> runningCommands = new ConcurrentHashMap<>();
    private Set<Command> defaultCommands = new HashSet<>();

    public Set<RobotStateListener> stateListeners = new HashSet<>();
...
//                        robotState);
//                logger.info("Finishing Command due to invalid state");
...
//                logger.trace("Command {} finished, it was cancelled?: {}", run.command.getName(), run.cancel);
    ...
    public void startWithContext(Command command, CommandRunContext context) throws CommandException {
        ...
        if (run != null || !command.isValidInState(robotState))
            //logger.info("Not in valid state for " + command.getName());
            return;
    ...
    public void printRunningCommands() {
        logger.info("Printing Commands: ");
        for (CommandRun cmd : runningCommands.values()) {

            logger.info("\t" + cmd.command.getName());
