...
import org.teamtators.common.commands.*;
...
//        LiveWindow.add(pdp); //FIXME
        SendableRegistry.remove(pdp);
...
//            getAutomatedTester().addTests(subsystem.createAutomatedTests());
    ...
    protected void deconfigureSubsystems() {
        ...
        pdp.close();
        LiveWindow.disableTelemetry(pdp); //FIXME
    ...
    protected void configureCommands() {
        ...
        var nodes = configLoader.load("Commands.yaml");
        if (nodes instanceof ObjectNode) {
            ObjectNode commandsConfig = (ObjectNode) nodes;
...
//        getAutomatedTester().configure(configLoader);
    }
...
//        }
    ...
    public FMSData getCurrentFMSData() {
        return fmsData;
    }
    ...
    protected void registerCommands(ConfigCommandStore commandStore) {
        ...
        commandStore.registerCommand("Conditional", () -> new ConditionalCommand(this));
    }
