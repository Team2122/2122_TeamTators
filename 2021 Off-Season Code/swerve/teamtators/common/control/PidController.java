...
public class PidController extends AbstractController implements Configurable<PidController.Config> {
    ...
    private double gravityComp = Double.NaN;
    ...
    public PidController(String name, double kP, double kI, double kD) {
        super(name);
        LiveWindow.enableTelemetry(this);
        setPID(kP, kI, kD);
    }
    ...
    protected double computeOutput(double delta) {
        ...
        if(!Double.isNaN(gravityComp)) {
            output += applyGravityCompensation(getInput());
        }
    ...
    private double applyGravityCompensation(double input) {
        return Math.sin(input) * gravityComp;
    }
    ...
    public void configure(Config config) {
        ...
        setGravityComp(config.gravityComp);
        super.configure(config);
    }

    public static class Config extends AbstractController.Config {
        public double P = 0.0, I = 0.0, D = 0.0, F = 0.0;
        public double maxIError = Double.POSITIVE_INFINITY, minISetpoint, gravityComp = Double.NaN;
    }
