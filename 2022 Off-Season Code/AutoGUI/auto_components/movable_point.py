...
class MovablePoint(Button):
    """Any point that the user can move and modify the values of the fields (ControlPoint, WayPoint, etc.)"""
    ...
    # Size
    base_length = get_measurement(SCREEN_LENGTH, 1.4)
    ...
    def __init__(self, left_edge, top_edge, base_color, selected_color, click_function, button_number):
        ...
        super().__init__(WINDOW, bg=base_color, compound=tkinter.CENTER, command=lambda: click_function(self),
                         text=button_number, fg=white, font=SMALL_FONT)
        #
        # MAKE SURE THIS IS BEFORE PLACING
        #
        self.user_modifiable_fields_grid = GridItems(self.user_modifiable_fields)
    ...
    def update_input_fields_belongs_to(self):
        """So the Main Screen can know what the input field's belong to (a ControlPoint for example)"""
