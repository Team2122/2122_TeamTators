...
public abstract class DeviceHealthManager {
	...
	public static void printHealth(RobotControlMode mode) {
		...
		if (currentlyHealthy) {
			...
			System.out.println("Current faults: " + currentFaults.substring(0, currentFaults.length()-2));
		}
		if (stickyHealthy) {
	...
	public static void logHealth() {
        ...
        if (!currentlyHealthy) {
            Logger.recordOutput("Health/Momentary/Faults", currentFaults.substring(0, currentFaults.length()-2));
        } else {
        ...
        if (!stickyHealthy) {
            Logger.recordOutput("Health/Sticky/Faults", stickyFaults.substring(0, stickyFaults.length()-2));
        } else {
