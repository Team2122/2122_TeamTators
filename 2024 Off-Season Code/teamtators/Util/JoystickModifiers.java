...
public class JoystickModifiers {
    ...
    public JoystickModifiers(double offset, double deadzone, double exponent, double cutoffX, double cutoffY) {
        this.offset = offset;
        this.deadzone = deadzone;
        this.exponent = exponent;
    ...
    public JoystickModifiers(double offset, double deadzone, double exponent, double cutoffX, double cutoffY, double limitY) {
        this.offset = offset;
        this.deadzone = deadzone;
        this.exponent = exponent;
        this.cutoffX = cutoffX;
    ...
    public double applyPiecewise(double input ) {
		...
		if(absInput < deadzone)
            ...
            out = sign * (cutoffY + ((1-cutoffY) * Math.pow((absInput - cutoffX)/(1-cutoffX), exponent )));
        }

        if(out > limitY) out = limitY;
