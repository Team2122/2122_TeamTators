...
public class TatorMath {
     ...
     * @param lookup the matrix of values where all values in f[a][0] are inputs and f[a][b]
     * are outputs, where a and b are arbitrary values, and a >= 0 && b > 0
     * @param index the index of the desired output in lookup
     *
     * e.g. lookup = {{1, 2, 0, 3},
     *                {4, 5, 0, 6},
     *                {7, 8, 0, 9}}
     * linearlyInterpolate(2, lookup, 3) will return 4.0
     *
     ...
     *                {4, 5, 0, 6},
     *                {7, 8, 0, 9}}
     * linearlyInterpolate(2, lookup, new int[] {1, 3}) will return {3.0, 4.0}
     *
     * @return The array of desired outputs. Will not include outputs from indexes that weren't specified in the indexes array
     */
