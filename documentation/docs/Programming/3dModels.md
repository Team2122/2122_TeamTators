...
### Movement
This portion requires a lot of information from the WPI methods section, linked [here](https://github.wpilib.org/allwpilib/docs/release/java/edu/wpi/first/math/geometry/Pose3d.html)
...
* Create a rotation 3D and set it to time
    * this will make the component rotate in a full circle, which helps to pinpoint the point of rotation
* Create a Pose 3d and set it to k.Zero, then make it rotate around (your subsystem's point of rotation, the value rotation 3d you just made) (for more context about .rotateAround(), see the WPI documentation)
    * Log said Pose 3d

Before you continue to find the point of rotation, make sure that your component is rotating about the part of the component you want it to. If it looks like it is rotating about a different part of the component, set your rotation 3d to 0 and configure.

* Once your component is rotating in a circular motion, use trial and error to move your component to the point on the robot you want it to rotate about. Start by making big changes (such as changing a coordinate value by 0.1) to your starting point, and as you get closer to the point, make smaller and smaller adjustments.

![RotationPointEx](../../\docs\images\RotationPoint.png)
this is an example of a rotation point. Notice both positive and negative values, as well as the fact that these numbers are pretty small. This is because the measurements are in meters, relative to the bottom center of the robot.

#### Rotation - Making it move as it does in real life

* Now, set your rotation 3d so that one of the values (roll, pitch, or yaw) is set to inputs.currentPosition * 2pi.
    * It is multiplied by 2pi because that is the equation to convert rotations to radians 
...
* Use trial and error to find the starting point, beginning with large adjustments in the values and slowly refining
...
* Simply add inputs.currentPosition to whichever one of the startingPoint.get X(), Y(), or Z() values
    * this will often make it move in either direction far too much. To solve this, use trial and error to find a coefficient to inputs.currentPosition that will make it move less.
